...
public:
	...
	string getAddress() {
		...
		return to_string((long long int)id) + ":" + to_string((long long int)port);
	}
	void init() {
...
public:
	...
	long timestamp;
    bool failed;
	MemberListEntry(int id, short port, long heartbeat, long timestamp, bool failed);
	MemberListEntry(int id, short port);
	MemberListEntry(): id(0), port(0), heartbeat(0), timestamp(0), failed(false) {}
	MemberListEntry(const MemberListEntry &anotherMLE);
	MemberListEntry& operator =(const MemberListEntry &anotherMLE);
	bool operator <(const MemberListEntry &anotherMLE) const { return id < anotherMLE.id; }
